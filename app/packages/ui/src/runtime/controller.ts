 ...
 *
 *   ┌──────────────────────────────────────────────────────────────────┐
 *   │ Managed — ws-ui's built-in themes (`{brand}-{mode}` contract)   │
 *   │   setPref / getPref     — user preference: auto | light | dark   │
 *   │   setBrand / getBrand   — brand string (ws-ui ships "default")  │
 *   └──────────────────────────────────────────────────────────────────┘
 *   ┌──────────────────────────────────────────────────────────────────┐
 *   │ Escape hatch — for themes ws-ui doesn't know about              │
 ...
 *   │                                                                  │
 *   │ Use cases:                                                       │
 *   │   - a theme from defineTheme() ("ocean", user-defined)         │
 *   │   - consumer-side brands not built into ws-ui CSS               │
...
export interface ThemeControllerOptions {
  brand?: string
  /** localStorage key for the user's mode pref. Default: 'hf:ws-ui:theme-pref'. */
  storageKey?: string
  /** Storage key for the pinned override. Default: 'hf:ws-ui:theme-override'. */
...
export class ThemeController {
  ...
  constructor({
    ...
    // override — otherwise an override pointing at "ocean" would apply
  ...
  // ────────────────────────────────────────────────────────────────────
  // Escape hatch — opaque-to-ws-ui theme value
  // ────────────────────────────────────────────────────────────────────
   ...
   *
   * Use for:
   *   - dynamic themes defined via `defineTheme()` (e.g. "ocean")
   *   - consumer-side brand themes not built into ws-ui
