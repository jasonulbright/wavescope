  ...
  it('builds the row anatomy from start / title / subtitle / end slots', () => {
    render(
      <Dropdown.Root defaultOpen>
        <Dropdown.Trigger>Open</Dropdown.Trigger>
        <Dropdown.Content>
          <Dropdown.Item start={<FolderIcon />} title="Aurora 2.0" subtitle="Ultra-real" end="2,482" />
        </Dropdown.Content>
      </Dropdown.Root>,
    )
    const item = screen.getByRole('menuitem', { name: /Aurora 2.0/ })
    expect(item.querySelector('.q-menu-item-icon')).toBeInTheDocument()
    expect(item.querySelector('.q-menu-item-title')).toHaveTextContent('Aurora 2.0')
    ...
    render(
      <Dropdown.Root defaultOpen>
        ...
        <Dropdown.Content withSearch>
          ...
          <Dropdown.Item value="cascade">
            <Dropdown.ItemContent><Dropdown.ItemTitleRow><Dropdown.ItemTitle>Cascade 3.0</Dropdown.ItemTitle></Dropdown.ItemTitleRow></Dropdown.ItemContent>
          </Dropdown.Item>
        </Dropdown.Content>
      </Dropdown.Root>,
    )
    await user.type(screen.getByPlaceholderText('Search'), 'cascade')
    expect(screen.getByText('Cascade 3.0')).toBeInTheDocument()
