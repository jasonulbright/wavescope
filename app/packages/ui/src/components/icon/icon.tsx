...
/**
 * Icon — paints a glyph from `@wavescope/icons` (or any SVG component) with
 * ws-ui's standardized size, color, and a11y. It replaces the scattered
 * `& svg { width/height }` rules across components: every consumer sizes via the
 * `--ws-icon-*` token scale (xs..xl -> 12/16/20/24/28px) and inherits color from
 ...
 *
 *   import { IconMagnifyingGlassOutlined } from '@wavescope/icons/IconMagnifyingGlassOutlined'
 ...
 *
 * Sizing strategy (icon.css): `q-icon` sets `width`/`height: var(--ws-icon-<size>)`
 ...
 * DOM node only for glyphs that forward their ref; the `@wavescope/icons`
...
/**
 * ws-ui icon color tokens -> `text-q-icon-*` utilities. Default is `currentColor`
...
/** A glyph component shape (matches every `@wavescope/icons/<Icon>` export). */
export type IconGlyph = ComponentType<SVGProps<SVGSVGElement>>

export type IconProps = {
  /** Token size (default `md` = 20px). Maps to `var(--ws-icon-<size>)`. */
  size?: IconSize
  /** ws-ui icon color token. Omit to inherit `currentColor`. */
  color?: IconColor
  /**
...
export function Icon({
  ...
  // sizes the svg via --ws-icon-*; color flows through `currentColor`.
