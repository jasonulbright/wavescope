/**
 * Material Symbols replacement for `@wavescope/icons` (template-only).
 *
 * The vendored `@wavescope/ui` components import their glyphs from the
 * private, Nexus-only `@wavescope/icons` package. Generated websites build on
 * the PUBLIC npm registry and must not depend on the internal registry, so
 * `@wavescope/icons/*` is aliased to THIS module (see `vite.config.ts`
 * `resolve.alias` + `tsconfig.json` paths). Each export mirrors the
 * `@wavescope/icons/<Name>` subpath a ws-ui component imports, mapped to its
 ...
 * `fill: currentColor`, so it drops straight into ws-ui's `<Icon as={…}>`
 ...
 * read solid. Only the generic UI glyphs the SHIPPED ws-ui components use are
 * mapped; if a future ws-ui sync adds a new glyph to a component, add it here
 * (the build fails with an unresolved export otherwise).
 */
import type { IconGlyph } from "@wavescope/ui/icon";
...
// app's @types/react while ws-ui types glyphs against its own vendored copy
// (`IconGlyph`). Runtime is unaffected — the glyphs just spread props onto
// the <svg> — so re-export each icon cast to ws-ui's IconGlyph.
