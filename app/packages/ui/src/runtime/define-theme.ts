 ...
 * populating --ws-color-* storage. Tailwind utilities pick up the values
 ...
 * `document.documentElement.dataset.theme = "ocean"` — but then the
...
// text VERBATIM — and themes can come from untrusted input, so treat all three
...
function injectStyleTag(name: string, tokens: ThemeTokens, options: InjectStyleTagOptions = {}): void {
  ...
  // `@layer ws-ui-theme`, while this runtime-injected style is unlayered. Per the
