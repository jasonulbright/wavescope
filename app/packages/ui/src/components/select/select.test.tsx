...
import { IconFolder1Outlined as FolderIcon } from '@wavescope/icons/IconFolder1Outlined'
...
const MODEL_ITEMS = { aurora: 'Aurora 2.0', prism: 'Prism Image 2', cascade: 'Cascade 3.0' }
    ...
    <Select.Root defaultOpen={defaultOpen} items={MODEL_ITEMS} {...rootProps}>
      ...
      <Select.Content>
        <Select.Group>
          ...
          <Option value="aurora" label="Aurora 2.0" />
          <Option value="prism" label="Prism Image 2" />
          <Option value="cascade" label="Cascade 3.0" />
        </Select.Group>
      </Select.Content>
    </Select.Root>
  )
...
describe('Select popup', () => {
  ...
  it('renders a group label and option rows', () => {
    ...
    expect(screen.getByRole('option', { name: 'Aurora 2.0' })).toHaveClass('q-menu-item', 'q-select-item')
  })
...
describe('Select selection', () => {
  it('seeds the trigger value from defaultValue', () => {
    render(<Basic defaultValue="prism" />)
    expect(screen.getByRole('combobox')).toHaveTextContent('Prism Image 2')
  ...
  it('selects an option on click and echoes it into the trigger', async () => {
    ...
    await user.click(await screen.findByRole('option', { name: 'Cascade 3.0' }))
    expect(screen.getByRole('combobox')).toHaveTextContent('Cascade 3.0')
  })
  ...
  it('fires onValueChange with the chosen value', async () => {
    ...
    await user.click(screen.getByRole('combobox'))
    await user.click(await screen.findByRole('option', { name: 'Aurora 2.0' }))
    expect(onValueChange).toHaveBeenCalledWith('aurora', expect.anything())
  })

  it('marks the selected option with data-selected', () => {
    render(<Basic defaultOpen defaultValue="aurora" />)
    expect(screen.getByRole('option', { name: 'Aurora 2.0' })).toHaveAttribute('data-selected')
    expect(screen.getByRole('option', { name: 'Prism Image 2' })).not.toHaveAttribute('data-selected')
  })
