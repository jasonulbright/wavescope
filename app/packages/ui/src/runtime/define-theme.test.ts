...
describe('defineTheme — injection guard', () => {
  it('injects a :where block for valid tokens (the happy path still works)', () => {
    defineTheme('ocean', {
      ...
      'brand-primary': 'var(--ws-color-fallback, #fbbf24)',
      'glow': 'linear-gradient(90deg, #000 0%, #fff 100%)',
    }, { persist: false })

    const tag = document.getElementById('ws-runtime-theme-ocean')
    expect(tag?.textContent).toContain(':where([data-theme="ocean"])')
    expect(tag?.textContent).toContain('--ws-color-glow: linear-gradient(90deg, #000 0%, #fff 100%);')
  })
  ...
  it.each([
    ...
    expect(document.getElementById('ws-runtime-theme-attack')).toBeNull()
  ...
  it('a bad redefinition does NOT destroy the valid theme it tried to replace', () => {
    ...
    expect(document.getElementById('ws-runtime-theme-keeper')).not.toBeNull()
  })
...
describe('hydratePersistedThemes — poisoned storage', () => {
  it('skips an entry someone wrote past the guard, hydrates the rest', () => {
    ...
    expect(document.getElementById('ws-runtime-theme-evil')).toBeNull()
    expect(document.getElementById('ws-runtime-theme-fine')).not.toBeNull()
  })
})
