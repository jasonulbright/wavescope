...
type ButtonProps<E extends ElementType> = ButtonOptions & {
  ...
  children?: ReactNode
  /**
   * Leading slot before the label — defaults to nothing; pass any node (a ws-ui
...
/**
 * Button — composite + slot-friendly: every part is a replaceable node with a
 * sensible default (nothing). The label is `children`; `start` / `end` are slots
 * that take any node, so you compose OTHER ws-ui components into the button
