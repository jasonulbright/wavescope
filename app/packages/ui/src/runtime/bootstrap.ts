 ...
 *      "ocean" would apply before :where([data-theme="ocean"])
 *      exists in the DOM.
 ...
 *
 * Usage (Next.js):
 *   import { bootstrapScript } from "@wavescope/ui/runtime";
 *
...
export interface BootstrapOptions {
  ...
  storageKey?: string
  /** localStorage key for the pinned override. Default 'hf:ws-ui:theme-override'. */
  overrideStorageKey?: string
  /** localStorage key for the runtime themes map. Default 'hf:ws-ui:runtime-themes'. */
...
export function bootstrapScript({
  ...
  for (var name in themes) {
    ...
    for (var key in themes[name]) {
      ...
      decls += "  --ws-color-" + key + ": " + themes[name][key] + ";\\n";
    }
    var style = document.createElement("style");
    style.id = "ws-runtime-theme-" + name;
    ...
    // @layer ws-ui-theme — unlayered always beats layered per CSS spec.
